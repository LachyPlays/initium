...
int main(int argc, char* argv[])
{
  ...
  {
    extensions[i] = glfw_extensions_raw[i];
  };
  ...
  auto instance_result =
      initium::createInstance({.application_name = "Suzanne test",
  ...
  VkDebugUtilsMessengerEXT debug_messenger =
      instance_result.value().debug_messenger;
  ...
  // Swapchain request
  VkSurfaceFormatKHR swapchain_format = {.format = VK_FORMAT_R8G8B8A8_SRGB,
                                         .colorSpace =
  ...
  auto physical_device =
      initium::pick_physical_device(
          instance, {.queue_requests = &queue_requests,
                     .formats = {},
                     .features = {},
                     .limits = {},
  ...
  auto device_result = initium::createLogicalDevice(
      physical_device, {}, {VK_KHR_SWAPCHAIN_EXTENSION_NAME}, queue_requests,
      swapchain_requests, true);
  auto device = device_result.value();
  if (!queue_requests[0].queue.has_value())
  {
    std::cerr << "Failed to create graphics queue" << std::endl;
    return 1;
  }
  if (!queue_requests[1].queue.has_value())
  {
  ...
  if (!swapchain_requests[0].swap_chain.has_value())
  {
  ...
  // Shaders
  auto vert_shader_result =
      initium::loadShaderModule(device, "shaders/shader.vert.spv");
  if (!vert_shader_result.has_value())
  {
    std::cerr << "Failed to load vertex shader";
    return 1;
  }
  auto frag_shader_result =
      initium::loadShaderModule(device, "shaders/shader.frag.spv");
  ...
  initium::ShaderDefine vert_define = {.module = vert_shader,
                                       .stage = VK_SHADER_STAGE_VERTEX_BIT,
                                       .entrypoint_name = "main"};
  initium::ShaderDefine frag_define = {.module = frag_shader,
  ...
  // Vertex feeding and model loading
  ...
  VkBuffer vertex_buffer =
      initium::createBuffer(device, vertex_buffer_params).value();
  VkDeviceMemory vertex_memory =
      initium::createBufferAllocation(device, physical_device, vertex_buffer,
  ...
  VkBuffer staging_buffer =
      initium::createBuffer(device, staging_buffer_params).value();
  VkDeviceMemory staging_memory =
      initium::createBufferAllocation(device, physical_device, staging_buffer,
  ...
  if (initium::recordCommandBuffer(
           ...
           indice_bytes, staging_buffer](VkCommandBuffer command_buffer)
          {
            ...
            vkCmdCopyBuffer(command_buffer, staging_buffer, vertex_buffer, 1,
                            &vertex_region);
            VkBufferCopy index_region = {.srcOffset = vertice_bytes,
                                         .dstOffset = 0,
                                         .size = indice_bytes};
  ...
  initium::ViewportParams viewport_params = {.width = (float)surface_width,
                                             .height = (float)surface_height};

  initium::ScissorParams scissor_params = {.width = (unsigned int)surface_width,
                                           .height =
  ...
  auto framebuffers_result =
      initium::createFramebufferFromViews(device, framebuffer_params);
  ...
  auto command_buffer_result = initium::allocateCommandBuffer(
  ...
  for (int i = 0; i < max_frames_in_flight; i++)
  {
    if (vkCreateSemaphore(device, &sem_create_info, nullptr,
                          &image_available[i]) != VK_SUCCESS)
    {
      printf("Failed to create image availability semaphore(s)\n");
      return 1;
    }
  }
  for (int i = 0; i < swapchain_image_count; i++)
  {
    ...
    {
      printf("Failed to create render completion semaphore(s)\n");
      return 1;
    }
  }
  ...
  for (int i = 0; i < max_frames_in_flight; i++)
  {
    if (vkCreateFence(device, &fen_create_info, nullptr, &in_flight[i]) !=
        VK_SUCCESS)
    {
  ...
  while (!glfwWindowShouldClose(window))
  {
    ...
    submit_info.pSignalSemaphores = &render_finished[free_image_index];
    submit_info.pWaitDstStageMask = &wait_stages;

    if (vkQueueSubmit(graphics_queue, 1, &submit_info, in_flight[i_f_index]) !=
        VK_SUCCESS)
    {
    ...
    std::vector<VkSemaphore> present_wait_sems = {
        render_finished[free_image_index]};

    VkPresentInfoKHR present_info{};
    present_info.sType = VK_STRUCTURE_TYPE_PRESENT_INFO_KHR;
    present_info.waitSemaphoreCount = present_wait_sems.size();
    present_info.pWaitSemaphores = present_wait_sems.data();
  ...
  vkFreeCommandBuffers(device, command_pool, max_frames_in_flight,
